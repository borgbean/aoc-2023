 ...
 * @returns 
 */
function doIt(input, start, dir, ord) {
    let h = input.length;
...
function doIt2(input) {
    let max = 0;
    input = input.split('\n');
    
    for(let i = 0; i < input.length; ++i) {
        max = Math.max(max, doIt(input, [i, -1], [0, 1], RIGHT));
        max = Math.max(max, doIt(input, [i, input[0].length], [0, -1], LEFT));
    }
    for(let j = 0; j < input[0].length; ++j) {
        max = Math.max(max, doIt(input, [-1, j], [1, 0], DOWN));
        max = Math.max(max, doIt(input, [input.length, j], [-1, 0], UP));
    }
